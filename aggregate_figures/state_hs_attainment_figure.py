"""
State-Level High School Attainment by Region

This script creates high school attainment trends by region using pre-aggregated
state-level cohort data. It mirrors the structure of state_college_attainment_figure.py.
"""
...
def aggregate_to_regions(df):
    ...
    --------
    pd.DataFrame
        Regional aggregates with recalculated hs_or_more_rate
    """
    ...
    # Calculate HS or more rate from aggregated counts
    regional_data['hs_or_more_rate'] = (regional_data['hs_or_more_count'] /
                                         regional_data['total_count']) * 100

    # Calculate HS only rate (HS or more - college)
    regional_data['hs_only_count'] = regional_data['hs_or_more_count'] - regional_data['college_count']
    regional_data['hs_rate'] = (regional_data['hs_only_count'] /
                                 regional_data['total_count']) * 100
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Create high school attainment by region figure from state-level data.'
    )
    ...
    print(f"Overall HS attainment rate (HS only): {regional_data['hs_only_count'].sum() / regional_data['total_count'].sum() * 100:.2f}%")
    ...
    for region in ['Northeast', 'Midwest', 'South', 'West']:
        ...
        if len(region_data) > 0:
            total_hs_only = region_data['hs_only_count'].sum()
            total_pop = region_data['total_count'].sum()
            rate = (total_hs_only / total_pop * 100) if total_pop > 0 else 0
            print(f"  {region}: {rate:.2f}% HS attainment ({total_pop:,} individuals)")
    ...
    # ========================================================================
    # Plot 1: Raw High School Attainment Rates by Region
    # ========================================================================
    print("\n" + "="*60)
    print("Creating raw high school attainment rate plot")
    print("="*60)
    ...
    for region in ['Northeast', 'Midwest', 'South', 'West']:
        region_data = regional_data[regional_data['region'] == region]
        if len(region_data) > 0:
            ax.plot(region_data['cohort_5year'], region_data['hs_rate'],
                   linewidth=2, marker='o', markersize=5, label=region,
                   color=region_colors[region])

    ax.set_xlabel('High School Cohort (5-Year Bins)', fontsize=12)
    ax.set_ylabel('High School Attainment Rate (%)', fontsize=12)
    ax.set_title(f'High School Attainment by High School Cohort and Region, {args.cohort_min}-{args.cohort_max}',
                fontsize=14)
    ...
    raw_output_path = os.path.join(args.output, "state_hs_attainment_by_cohort_region_raw.png")
    ...
    formula = 'hs_rate ~ C(region_cat, Treatment(reference="Northeast")) * C(cohort_cat, Treatment(reference=1900))'
    ...
    ax.set_title('Regional Differences in High School Attainment Across Cohorts\n(Regression Coefficients: Region × Cohort Interactions)',
                fontsize=14)
    ...
    coef_output_path = os.path.join(args.output, "state_hs_attainment_regression_coefficients.png")
