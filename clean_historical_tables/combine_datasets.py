...
def get_dataset_info(df: pd.DataFrame, filename: str) -> Dict:
    ...
    if 'college' in df.columns:
        ...
        # Also create the university-style names for backward compatibility
        info['unique_college_university_pairs'] = colleges.nunique()
        info['total_college_university_records'] = len(colleges)
    else:
        info['unique_colleges'] = 0
        info['total_college_records'] = 0
        info['unique_college_university_pairs'] = 0
        info['total_college_university_records'] = 0
...
def create_summary_markdown(datasets_info: Dict, variable_analysis: Dict, output_path: str):
    ...
    markdown_content.append("")
    markdown_content.append("| Dataset | Year | Rows | Columns | Unique College-University Pairs | File |")
    markdown_content.append("|---------|------|------|---------|--------------------------------|------|")
    ...
    for dataset_name, info in sorted_datasets:
        ...
        unique_college_uni_pairs = info.get('unique_college_university_pairs', 0)
        markdown_content.append(f"| {dataset_name} | {year} | {rows:,} | {cols} | {unique_college_uni_pairs:,} | {filename} |")
    
    markdown_content.append("")
    
    # College-University pair statistics summary
    total_college_uni_pairs_across_datasets = sum(info.get('unique_college_university_pairs', 0) for info in datasets_info.values())
    total_college_uni_records = sum(info.get('total_college_university_records', 0) for info in datasets_info.values())
    
    markdown_content.append("## College Statistics Summary")
    markdown_content.append("")
    markdown_content.append(f"- **Total unique colleges across all datasets:** {total_college_uni_pairs_across_datasets:,}")
    markdown_content.append(f"- **Total college-related records:** {total_college_uni_records:,}")
    markdown_content.append("")
