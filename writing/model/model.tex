...
Estimation of education costs:

\begin{equation}
    \log \tilde w_{jkl} - \log \bar e_{jk} = \log\frac{\tilde \epsilon}{\bar\epsilon \eta} - \log a_l + \log \tau_{jk}^e + \log \tau_{jkl}^m
\end{equation}

which allows for recovery of education costs $\tau_{jk}^e$

$\tau_{jk}^e$ is an origin by age group fixed effect. In the model is has the interpretation of an education cost which we will decompose

We then take these education costs and decompose them:

\begin{equation}
    \log \tau_{jk}^e = \alpha_j +\alpha_k +\beta S_j T_k + C_jT_k\phi +\epsilon_{jk}
\end{equation}

18 is substituted into 17 and estimated.
...
We can similarly decompose education:

\begin{equation}
    \log \bar e_{jk} = \tilde \alpha_j + \tilde\alpha
\end{equation}



Here we are using the quasiexperimental variation in the school construction that varies over the age cohorts as an instrument for these costs.
